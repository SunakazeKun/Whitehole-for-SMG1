...
public class BmdRenderer extends GLRenderer {
    private void uploadTexture(GL2 gl, int id) {
        ...
        for (int i = 0; i < tex.mipmapCount; i++)
            hash = (int) Utils.superFastHash(tex.image[i], (long)hash, 0, tex.image[i].length);
        textures[id] = hash;
    ...
    private int shaderHash(int matid) {
        ...
        return (int) Utils.superFastHash(sigarray, 0, 0, sig.position());
    }
    ...
    private void generateShaders(GL2 gl, int matid) throws GLException
    {
        ...
        for (int i = 0; i < mat.numTexgens; i++)
        {
            ...
            if (mtxid >= 30 && mtxid <= 57)
            {
                ...
                {
                    //if (j > 0) thematrix += ",";
                   // thematrix += String.format(usa, "%1$f", texmtx.basicMatrix.matrix[j]);
                    vert.append(String.format(usa, "%2$s%1$f", texmtx.basicMatrix.matrix[j], (j>0)?",":""));
                }
                //thematrix += ")";
                vert.append(");\n");
    ...
    protected final void ctor_loadModel(RenderInfo info, String modelname) throws GLException {
        ...
        try {
            ...
            if (!Whitehole.game.filesystem.fileExists(filepath))
                filepath = "MapPartsData/" + modelname + ".arc";
            if (!Whitehole.game.filesystem.fileExists(filepath))
                throw new GLException("Failed to locate archive for " + modelname);
            
            container = new RarcFilesystem(Whitehole.game.filesystem.openFile(filepath));
            
            String modelpath = '/' + modelname + '/' + modelname + ".bdl";
            if (!container.fileExists(modelpath))
                modelpath = '/' + modelname + '/' + modelname + ".bmd";
            if (!container.fileExists(modelpath))
                throw new GLException("Failed to load model for " + modelname);
            
            model = new Bmd(container.openFile(modelpath));
            
            String shapepath = '/' + modelname + "Wait.bva";
            if (!container.fileExists(shapepath))
                shapepath = '/' + modelname + "Normal.bva";
            if (!container.fileExists(shapepath))
                return;
            
            visible = new Bva(container.openFile(shapepath));
        }
            ...
            throw new GLException("Failed to load model data for " + modelname);
        }
    }
    ...
    protected final void ctor_uploadData(RenderInfo info) throws GLException {
        ...
        hasShaders = hasShaders && Settings.applyShaders;
