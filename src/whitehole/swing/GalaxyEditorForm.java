...
public class GalaxyEditorForm extends javax.swing.JFrame {
    ...
    public GalaxyEditorForm(String galaxy) {
        ...
        try {
            ...
            zoneArcs = new HashMap<>(galaxyArc.zones.size());
            for (String zone : galaxyArc.zones)
                loadZone(zone);
            
            ZoneArchive mainzone = zoneArcs.get(galaxyName);
            for (int i = 0; i < galaxyArc.scenarios.size(); i++) {
                ...
                int mainlayermask = (int)galaxyArc.scenarios.get(i).layers.get(galaxyName);
                for (int l = 0; l < 16; l++) {
    ...
    private void initGUI() {
        ...
        String[] items = new String[] { "Normal", "Child", "MapPart", "Spawn", "Gravity", "Area", "Camera", "Sound", "Cutscene", "Position", "Debug", "Path", "Path point" };
    ...
    private void loadZone(String zone) throws IOException {
        ...
        for (java.util.List<TObject> objlist : arc.objects.values()) {
    ...
    {//GEN-HEADEREND:event_formWindowOpened
        if (galaxyMode) {
            ...
            for (GalaxyArchive.Scenario scenario : galaxyArc.scenarios)
                scenlist.addElement(scenario.toString());
    ...
    public void selectionChanged() {
        ...
        for (TObject obj : selectedObjs.values()) {
            int pathid = -1;
        ...
        if (selectedObjs.size() > 1) {
            for (TObject selectedObj : selectedObjs.values()) {
            ...
            for (TObject selectedObj : selectedObjs.values()) {
                if (selectedObj instanceof PathPointObj) {
                    ...
                    if (selectedObj.getClass() != PathPointObj.class) {
                        ...
                        if (selectedObj.getClass() != StartObj.class
                                && selectedObj.getClass() != SoundObj.class
                                && selectedObj.getClass() != PositionObj.class
                                && selectedObj.getClass() != DebugObj.class)
                            pnlObjectSettings.addField("name", "Object", "objname", null, selectedObj.name, "Default");
                        if (galaxyMode)
                            pnlObjectSettings.addField("zone", "Zone", "list", galaxyArc.zones, selectedObj.zone.zoneName, "Default");
    ...
    private void populateObjectSublist(int layermask, ObjListTreeNode objnode, Class type) {
        for (java.util.List<TObject> objs : curZoneArc.objects.values()) {
    ...
    private void tvObjectListValueChanged(javax.swing.event.TreeSelectionEvent evt) {//GEN-FIRST:event_tvObjectListValueChanged
        ...
        for (TreePath path : paths) {
            ...
            if (!(tnode.object instanceof TObject))
                continue;

            TObject obj = (TObject)tnode.object;
    ...
    private void tgbDeleteObjectActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_tgbDeleteObjectActionPerformed
        if (!selectedObjs.isEmpty()) {
            if (tgbDeleteObject.isSelected()) {
                Collection<TObject> templist = ((HashMap)selectedObjs.clone()).values();
                for (TObject selectedObj : templist)
                {
    ...
    private void lbZoneListValueChanged(javax.swing.event.ListSelectionEvent evt) {//GEN-FIRST:event_lbZoneListValueChanged
        ...
        curZone = galaxyArc.zones.get(selid);
        curZoneArc = zoneArcs.get(curZone);

        int layermask = (int) curScenario.layers.get(curZone);
    ...
    private void lbScenarioListValueChanged(javax.swing.event.ListSelectionEvent evt) {//GEN-FIRST:event_lbScenarioListValueChanged
        ...
        curScenario = galaxyArc.scenarios.get(curScenarioID);
        ...
        for (String zone : galaxyArc.zones)
        {
            ...
            int layermask = (int) curScenario.layers.get(zone);
    ...
    private void itemPositionActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_itemPositionActionPerformed
        if (selectedObjs.size() == 1) {
            for (TObject selectedObj : selectedObjs.values()) {
                copyPos = (Vector3) new Vector3(selectedObj.position);
    ...
    private void itemScalePasteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_itemScalePasteActionPerformed
        for (TObject selectedObj : selectedObjs.values()) {
            ...
            selectedObj.scale = new Vector3(copyScale);
    ...
    private void itemPositionPasteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_itemPositionPasteActionPerformed
        for (TObject selectedObj : selectedObjs.values()) {
            ...
            if (selectedObj instanceof PathPointObj) {
                ...
                Vector3 oldpos = new Vector3(selectedPathPoint.position);
                Vector3 newpos = new Vector3(copyPos);
                ...
                selectedObj.position = new Vector3(copyPos);
    ...
    private void itemRotationPasteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_itemRotationPasteActionPerformed
        for (TObject selectedObj : selectedObjs.values()) {
            ...
            selectedObj.rotation = new Vector3(copyDir);
    ...
    private void addObject(Point where) {
        ...
        if (objtype.equals("path") || (objtype.equals("pathpoint"))) {
            if (objtype.equals("path")) {
                ...
                DefaultTreeModel objlist = (DefaultTreeModel) tvObjectList.getModel();
                ObjListTreeNode listnode = (ObjListTreeNode) ((DefaultMutableTreeNode) objlist.getRoot()).getChildAt(11);
        ...
        switch (objtype) {
            ...
            case "camera": 
                ...
                break;
            case "sound":
                newobj = new SoundObj(curZoneArc, addingObjectOnLayer, objname, pos);
                nodeid = 7;
                break;
            case "cutscene": 
            ...
            case "position": 
                ...
                break;
            case "debug":
                newobj = new DebugObj(curZoneArc, addingObjectOnLayer, objname, pos);
                nodeid = 10;
                break;
            default:
                return;
    ...
    private void setObjectBeingAdded(String type) {         
        switch (type) {
            case "start":
                ...
                break;
            case "sound":
                addingObject = "sound|SoundEmitter";
                addingObjectOnLayer = "common";
                break;
            case "position":
            ...
            case "pathpoint":
                ...
                break;
            default:
                ObjectSelectDialog form = new ObjectSelectDialog(this, null);
                form.setVisible(true);
    ...
    private void deleteObject(int uid) {
        if (globalObjList.containsKey(uid)) {
            TObject obj = globalObjList.get(uid);
        ...
        for (TObject selectedObj : selectedObjs.values()) {
            ...
            // Any other object
                ...
                if (propname.equals("name")) {
                    ...
                    for (int z = 0; z < galaxyArc.zones.size(); z++) {
                        if (!galaxyArc.zones.get(z).equals(newzone))
                            continue;
    ...
    public class GalaxyRenderer implements GLEventListener, MouseListener, MouseMotionListener, MouseWheelListener, KeyListener
    {
        public class AsyncPrerenderer implements Runnable
        {
            ...
            public void run()
            {
                ...
                if (parentForm == null)
                {
                    for (TObject obj : globalObjList.values())
                    {
        ...
        public void init(GLAutoDrawable glad)
        {
            ...
            StartObj start = null;
            for (TObject obj : firstzone.objects.get("common"))
            {
            ...
            for (int s = 0; s < (galaxyMode ? galaxyArc.scenarios.size() : 1); s++)
        ...
        private void renderAllObjects(GL2 gl)
        {
            ...
            if (galaxyMode)
            {
                ...
                for (int s = 0; s < galaxyArc.scenarios.size(); s++)
                {
                   // if (!zoneDisplayLists.containsKey(s))
                    ...
                    GalaxyArchive.Scenario scenario = galaxyArc.scenarios.get(s);
                    renderZone(gl, scenario, galaxyName, (int)scenario.layers.get(galaxyName), 0);
        ...
        private void prerenderZone(GL2 gl, String zone)
        {
            ...
            for (String layer : layers)
            {
                ...
                for (TObject obj : zonearc.objects.get(layer))
                {
                    if (mode == 0) 
                    {
        ...
        private void renderZone(GL2 gl, GalaxyArchive.Scenario scenario, String zone, int layermask, int level)
        {
            ...
            if (level < 5) {
                for (StageObj subzone : zoneArcs.get(zone).zones.get("common")) {
                    ...
                    renderZone(gl, scenario, zonename, (int)scenario.layers.get(zonename), level + 1);
                    ...
                    case "object":
                        {
                            ...
                            TObject obj = globalObjList.get(objid);
                            obj.closeRenderer(renderinfo);
                            obj.initRenderer(renderinfo);
                            obj.oldname = obj.name;
                        }
        ...
        public void display(GLAutoDrawable glad)
        {
            ...
            gl.glLoadMatrixf(modelViewMatrix.matrix, 0);
        ...
        public void mouseReleased(MouseEvent e) {
            ...
            if (isDragging) {
                isDragging = false;
                if (Settings.applyFastDrag) e.getComponent().repaint();
                return;
            }
            ...
            TObject theobject = globalObjList.get(objid);
            ...
            if (e.getButton() == MouseEvent.BUTTON3) {
                ...
                if (!addingObject.isEmpty()) {
                    ...
                    if (ctrlpressed) {
                        ...
                        LinkedHashMap<Integer, TObject> oldsel = null;
                        
                        if (!selectedObjs.isEmpty() && arg == oldarg) {
                            oldsel = (LinkedHashMap<Integer, TObject>)selectedObjs.clone();
                            
                            for (TObject unselobj : oldsel.values()) {
